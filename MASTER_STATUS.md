...
OK; run -> PASS (reproduced across three runs); git diff --check clean.
...
controlled LIVE passive shadow soak (SHADOW_RUNTIME_ENABLED=1,
...
Validation (all pass): py_compile engines/stream_manager.py +
core/passive_shadow_bootstrap.py OK; bootstrap test PASS (disabled no-worker;
enabled start+drain; kill switch stops worker; repeated start/stop safe);
SHADOW_RUNTIME_ENABLED unset AND explicitly "0" both verified to start no worker
via start_passive_shadow()/get_default_bootstrap(); git diff --check clean.
...
Scope: this commit adds ONLY the two bootstrap files (+ these docs). The
unrelated untracked/working-tree items (core/daily_session.py, live_rdm.py
pre-existing hunks, live_return_detection.py, observation_logger.py,
research/zone_mechanics_calculator.py, research artifacts, unrelated experiments)
were NOT staged.

Validation (all pass): py_compile bootstrap + bootstrap_test + replay soak OK;
bootstrap test PASS (disabled no-worker; enabled start+drain; kill switch stops
worker; repeated start/stop safe); replay soak import smoke OK (committed soak
now resolves bootstrap); git diff --check clean.

Result: the committed soak tool no longer depends on untracked code; the
committed tree is self-consistent.

Note on doc order: the prior (Codex) development cycle recorded the Phase 0E-1 /
0E-2 / 0E-3 and RDM_V2_PASSIVE_SHADOW_REPLAY_SOAK_PASS checkpoints appended at the
BOTTOM of this file; this repository-fix block is prepended at the top to restore
an accurate current-checkpoint pointer.
...
- **Flags default OFF** — disabled (flags.should_run() False) -> no-op, queue
  untouched (status DISABLED); the default emitter reads flags from env so the
  convenience emit() is inert until explicitly enabled.
...
Validation (all pass): py_compile emitter + test OK; shadow emitter test PASS
(disabled no-op; enabled enqueues; kill switch blocks; queue full drops without
blocking; bad record never raises; payload deep-copied; global_zone_key +
geometry_version generated); git diff --check clean.
...
- **feature flags default OFF** (core/shadow_safety/feature_flag.py) —
  FeatureFlags.from_env reads SHADOW_RUNTIME_ENABLED / SHADOW_DRY_RUN /
  SHADOW_SAMPLE_RATE; absent/garbage/unreadable -> OFF; only an explicit truthy
  value opts in. Master gate should_run() + deterministic should_sample().
- **kill switch / circuit breaker** (core/shadow_safety/kill_switch.py) —
  ...
  self-revives (only reset() un-latches); KillSwitch adds manual env (SHADOW_KILL)
...
- **isolated worker wrapper** (core/shadow_safety/isolated_worker.py) —
  IsolatedWorker.process() runs a handler behind a try/except BaseException
...
- **parity log writer confined to research/shadow_parity/**
  ...
  only inside research/shadow_parity/; any path resolving outside is rejected at
  construction.

Strictly:
- **No live tap** — live_rdm.py untouched; no tap line added.
- **No production imports** — nothing in core/research/tools/engines imports
  core.shadow_safety except the package itself.
- **No production behavior changed** — no dashboard, no RDM formulas, no Stage 2C,
  no production outputs.

Validation (all pass): py_compile all shadow safety modules + test OK; shadow
safety test PASS (flags default OFF; kill switch latches closed + auto-trips;
queue drops on full and never blocks; worker swallows + counts exceptions;
parity logger confined to research/shadow_parity/); git diff --check clean.
...
Status: STABLE CHECKPOINT — shadow-only, no production behavior changed. This is
the consolidation checkpoint for the entire RDM V2 shadow architecture phase.
...
### 1. Event-Driven Backbone (complete)
    Market Row
      ...
      -> Canonical Snapshot
Components:
- core/interaction_interpreter.py
- core/event_dispatcher.py
- core/mechanical_refresh_coordinator.py
...
- **Snapshot Identity Contract** — global_zone_key is the canonical snapshot
  identity; zone_id is metadata only; the same zone_id reused across sessions
  does NOT collide (keyed by global_zone_key).
- **Row Ordering Contract** — interpret_in_order(); InteractionState.
  previous_row_index is the ONLY watermark; row_index is authoritative;
  timestamp is informational only; duplicate row -> ROW_DUPLICATE; older row ->
  ROW_OUT_OF_ORDER; no events and no state mutation on rejected rows.
- **Restart / Durability Contract** — append-only ordered row log is the source
  of truth; persist-before-process; rebuild-from-history; the snapshot is a
...
Behavior: copy-on-write; immutable revisions; one atomic revision per commit;
previous revision preserved on failure; keyed by global_zone_key.
...
All adapters: pure mapping only; no calculations; NOT_AVAILABLE-aware;
alias-aware where needed; snapshot-compatible; no production consumers.

### 5. Shadow Integration Tests
- experiments/coordinator_snapshot_integration/shadow_test.py — validated:
  Coordinator -> Row Mechanics -> Snapshot; Coordinator -> Row Mechanics + Open
  Visit -> Snapshot; Completed Visit integration; Dynamic Mechanics integration;
  Prediction integration (gated by ALL(trajectory_dirty, prediction_dirty)).
- experiments/full_shadow_runtime/shadow_test.py — validated the full runtime:
  Market Row -> Interaction Interpreter -> Event Dispatcher -> Mechanical Refresh
  Coordinator -> Adapters -> Canonical Snapshot.
...
Validation (all pass): py_compile experiments/full_shadow_runtime/shadow_test.py
OK; full shadow runtime test PASS (6 scenarios: simple row update; VISIT_COMPLETED
with prediction; prediction pending; injected adapter failure; duplicate row;
out-of-order row); 8 RefreshPlans -> 7 committed revisions (one rolled back by
the injected failure); git diff --check clean.
