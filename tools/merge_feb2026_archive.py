"""
Merge archived per-day February 2026 BTCUSDT replay outputs into one
chronologically-ordered, globally re-indexed research dataset.

February's raw replay data was downloaded in ~5-day batches and archived
as 28 separate single-calendar-day folders under archives/BTCUSDT/. Each
batch restarts row_id/episode_id numbering at 1, so a naive concatenation
would produce colliding IDs. This script walks the 28 archived days for
2026-02-01 .. 2026-02-28 in chronological order and applies offset-based
re-indexing (matching the mechanism in tools/run_backtesting_chunks.py)
so that row_id and episode_id are globally unique and monotonic, and
start_row_id/end_row_id stay consistent with the new row_id space (this
last part matters here specifically because February's episodes have not
yet passed through episode research, which looks rows up by row_id).

Reads only from archives/. Writes only to a staging directory
(outputs/feb2026_merge_staging/). Does not download anything, does not
call the replay generator, does not touch raw aggtrades, does not modify
any formula, RDM, B12v2, Synthesis, Formation Model, or lifecycle logic.
"""
...
STAGING_DIR = ROOT / "outputs" / "feb2026_merge_staging"

FEB_DATES = [f"2026-02-{d:02d}" for d in range(1, 29)]

OBS_FILE = "historical_observation_rows.csv"
EP_FILE = "historical_replay_dashboard_v2_episodes.csv"

FEB_START = pd.Timestamp("2026-02-01 00:00:00", tz="UTC")
FEB_END = pd.Timestamp("2026-02-28 23:59:59.999999999", tz="UTC")
...
def main() -> None:
    ...
    day_paths = [(date, find_day_archive(date)) for date in FEB_DATES]
    ...
    for date, path in day_paths:
        ...
        # Enforce February-only boundary defensively (archives observed clean,
        # but the requirement is explicit: exclude Jan 31 / Mar 1 spillover).
        ts = parse_market_timestamp(obs_df["market_timestamp"])
        in_bounds = (ts >= FEB_START) & (ts <= FEB_END)
        ...
        # several such rows are directly referenced by start_row_id/end_row_id
        # in this day's episodes. A timestamp-only dedupe would both destroy
        # real data and break row_id referential integrity.
    ...
    all_in_bounds = bool(((ts_all >= FEB_START) & (ts_all <= FEB_END)).all())
    ...
    report = []
    report.append("# February 2026 Archive Merge — Verification Report\n\n")
    report.append(f"Generated: {generated_at}\n\n")
    report.append(f"Source: {len(day_paths)} archived day folders ({FEB_DATES[0]} .. {FEB_DATES[-1]})\n\n")
    ...
    report.append(f"- all timestamps within Feb 1 00:00:00 .. Feb 28 23:59:59 bounds: {all_in_bounds}\n")
    report.append(f"- rows dropped as out-of-February-bounds: {out_of_bounds_total}\n")
